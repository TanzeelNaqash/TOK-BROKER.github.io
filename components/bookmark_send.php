...
if(isset($_POST['save'])){
   if($user_id != ''){
      ...
      $bookmark_id = create_unique_id();
      $property_id = $_POST['property_id'];
      $property_id = htmlspecialchars($property_id);
      ...
      if($verify_saved->rowCount() > 0){
         ...
         $success_msg[] = 'removed from My Bookmarks!';
      }else{
         $insert_saved = $conn->prepare("INSERT INTO`saved`(id, property_id, user_id) VALUES(?,?,?)");
         $insert_saved->execute([$bookmark_id, $property_id, $user_id]);
         $success_msg[] = 'Itemization saved!';
      }
