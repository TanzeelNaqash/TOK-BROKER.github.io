...
<head>
    ...
    <title>Welcome to TOK-BROKER - get your best choice! - Filter Search</title>
</head>
...
<section class="filters" stylstyle="padding-bottom: 0;">
...
if(isset($_POST['h_search'])){
   ...
   $h_location = htmlspecialchars($h_location);
   $h_type = $_POST['h_type'];
   $h_type = htmlspecialchars($h_type);
   $h_offer = $_POST['h_offer'];
   $h_offer = htmlspecialchars($h_offer);
   // $h_min = $_POST['h_min'];
   // $h_min = htmlspecialchars($h_min);
   // $h_max = $_POST['h_max'];
   // $h_max = htmlspecialchars($h_max);
   ...
   $select_properties = $conn->prepare("SELECT * FROM `property` WHERE address LIKE '%{$location}%' AND type LIKE '%{$type}%' AND offer LIKE '%{$offer}%' AND bhk LIKE '%{$bhk}%' AND status LIKE '%{$status}%' AND furnished LIKE '%{$furnished}%'ORDER BY date DESC");
   $select_properties->execute();
...
<?php 
   if(isset($_POST['h_search']) or isset($_POST['filter_search'])){
      ...
      echo '<h1 class="heading">latest itemization</h1>';
   }
?>
